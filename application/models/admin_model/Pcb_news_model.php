...
class Pcb_news_model extends CI_Model
{
	...
	parent::__construct();
        ...
        $this->adv_world = $this->db->dbprefix('adv_world');
    }
    ...
    function getDeviceNotificationIds($type='Android')
    {
        $this->db->distinct();
		$this->db->select('push_id')->from($this->push_notification);
		if($type!=''){
    ...
    function getAllPcbNews($type=NULL,$limit=NULL)
    {
			...
			$this->db->select('coll.id collection_id,news.id newsType_id, news.display_name,coll.is_video, coll.title,description, coll.image,created_date');
...
if($type==NULL && $limit!=NULL){
	  ...
	  function getNewsList($type=NULL,$id,$limit=NULL,$start=NULL)
    {
			...
			$this->db->select('coll.id collection_id,pcb_news_id newsType_id, coll.title, coll.is_video,description,tag,coll.image,created_date');
			$this->db->from($this->news_collection.' AS coll');
			$this->db->where('pcb_news_id',$type);
    ...
    {
                        $this->db->distinct();
			$this->db->select('coll.id collection_id,pcb_news_id newsType_id,coll.is_video, coll.title, description,tag,coll.image,created_date');
			$this->db->from($this->news_collection.' AS coll');
			$this->db->limit(4);
			$this->db->where('id <',$id);
   ...
   function savePcbNews($data){
    ...
    function getPcbPollbyID($id){
        ...
        $this->db->select('question, option_a,option_b, option_c, option_d,cnt_a,cnt_b,cnt_c,cnt_d')
            ->from($this->pcb_poll);
        $this->db->where('id',$id);
    ...
    function updatePlugins($data,$plugin_name){

        $this->db->where('plugin_name', $plugin_name);
        $this->db->update('pcb_plugins',$data);
        return $this->db->affected_rows();
    }
    ...
    function savePollVote($id,$option,$push_id=''){

        $this->db->select("$option,push_ids")->from($this->pcb_poll);
        $this->db->where('id',$id);
        $option_cnt=0;
        $result = $this->db->get();
        if($result->num_rows()>0)
        ...
        $push_ids= $push_ids.",".$push_id;
         if( $push_ids[0] == ',' ) {
         $push_ids = substr($string,1);
         }
        $data=array($option=>$option_cnt,'push_ids'=>$push_ids);
        }
        $this->db->where('id', $id);
        $this->db->update($this->pcb_poll,$data);
...
WHERE
    ...
    public function getVideoInfo($id){
        $sql="SELECT *
FROM    pcb_video 
WHERE video_type=$id  order by id desc limit 4";
        $query = $this->db->query($sql);
        $CandidateInfo=array();
        if($query->num_rows()>0){
            foreach($query->result_array() as $row)
 ...
 function getVideoList($type=NULL,$id,$limit=NULL,$start=NULL)
    {
...
function getSelectedArea($push_id){

        $this->db->select('notification_alert')
	...
	function getDashboardDetailsAPI($type,$limit){
		...
		$this->db->select('coll.id collection_id,news.id newsType_id, coll.description,news.display_name,coll.is_video, coll.title, coll.image,created_date');
    ...
    {
        $this->db->distinct();
        $this->db->select('coll.id collection_id,coll.pcb_news_id newsType_id, coll.title,coll.is_video, description,tag,coll.image,created_date');
        if($limit!=NULL){
            $this->db->limit($limit);
        }
        $this->db->from($this->news_collection.' AS coll');		
    ...
    {
        $this->db->distinct();
        $this->db->select('coll.id collection_id,coll.pcb_news_id newsType_id, coll.title, description,tag,coll.image,coll.is_video,created_date');
        if($limit!=NULL){
            $this->db->limit($limit);
        }
        $this->db->from($this->news_collection.' AS coll');
...
,description,occupation,mobile_no,email,photo,party,age,reservation_name');
    ...
    function getAllCandidateInfo($ward_id=null){
		...
		$this->db->order_by('no_votes','desc');
        $query = $this->db->get();
        $CandidateInfo=array();
        if($query->num_rows()>0){
            foreach($query->result_array() as $row)
	...
	public function updatePushId($old_id,$new_id){
			$this->db->where('push_id', $old_id);
			$data=array('push_id'=>$new_id);
			$this->db->update($this->push_notification,$data);
			return $this->db->affected_rows();
	}
	
	 
   public function getAdvsertise($id)
   {
		$this->db->select('adv_type,image');
        $this->db->from($this->adv_world);
		$this->db->where('adv_type', $id);
        $query = $this->db->get();
        $data=array();
        if($query->num_rows()>0){
            foreach($query->result_array() as $row)
    ...
    public function getAllAdvsertisement($device = null)
    {
        $this->db->select('id,adv_type,image');
        $this->db->from($this->adv_world);
        $device = isset($device) ? $device : "WEB";
        $this->db->where('web_or_app', $device);
        $query = $this->db->get();
        $data=array();
        if($query->num_rows()>0){
            foreach($query->result_array() as $row)
    ...
    function deletePcbAdsById($id){

        $this->db->where('id', $id);
        $this->db->delete($this->adv_world);
        return $this->db->affected_rows();
    }

public function getVideoCount($type,$id){
	   $this->db->select('id');
	   $this->db->from($this->video.' AS coll');
	   $this->db->where('video_type',$type);
	   $this->db->where('id <= ',$id);
	   $query = $this->db->get();
	   return $query->num_rows();
 ...
 function updateNotificationSettings($id,$data){
       $this->db->where('push_id', $id);
       $this->db->update($this->push_notification,$data);
       return $this->db->affected_rows();
    }

     function getPcbLatestPoll(){

        $this->db->select('id,question, option_a,option_b, option_c, option_d,cnt_a,cnt_b,cnt_c,cnt_d')
            ->from($this->pcb_poll);
        $this->db->limit(1);
        $this->db->order_by("id", "desc");
