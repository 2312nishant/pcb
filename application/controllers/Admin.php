...
class Admin extends CI_Controller
{
 ...
 $this->load->helper("notification");
   ...
   public function sendNotification()
   {
     ...
     $type="iOS";
     $iOSIds = $this->pcb_news_model->getDeviceNotificationIds($type);
	 ...
	 $image =str_replace('-', '%2D',  str_replace(' ', '%20', $newDetails[0]['image']));
  
     $data = array('post_id'=>$newsId,'post_title'=>$title,'post_description'=>$description,'is_video'=>$newDetails[0]['is_video'],'post_image'=>utf8_encode($this->config->item('appPath').'uploads/'.NOTIFICATION.'/'.$image));     
     
 if(!empty($pushIds)){
      while(count($pushIds)>0){
          $sendIds = array_splice($pushIds,0,999);
          sendMessage($data,$sendIds);
         $arr[]= $this->sendOnesignalNotification($sendIds,$newsId,$title,$description,$image,$created_date);
      }
}

       file_put_contents('OSIds.txt', print_r($iOSIds,1).PHP_EOL, FILE_APPEND);
 if(!empty($iOSIds)){
      while(count($iOSIds)>0){
          $sendIds = array_splice($iOSIds,0,999);
          sendMessageiOS($data,$sendIds);
      }
}
	/* if(!empty($pushIds)){
    ...
    {
        $result['data'] = $this->pcb_news_model->getTopCollectionNews(3);
		$result['headline'] = 'ताज्या बातम्या';
    ...
    {
        $result = $this->pcb_news_model->getTopReadCollectionNews(4);
        echo(json_encode($result));
    }
   ...
   public  function updateCandidateVotes(){
    ...
    {
       $result = $this->pcb_news_model->getAllAdvsertisement();
       echo json_encode($result);
    ...
    public function deleteAdv()
    {
        ...
        if (file_exists("uploads/".$request->adv_type.'/'.$request->image)) {
            unlink("uploads/".$request->adv_type.'/'.$request->image);
        }

        $news = $this->pcb_news_model->deletePcbAdsById($request->id);
        echo($news);
 ...
 public function updatePlugins()
    {
        ...
        if(isset($plugin_name) && isset($status)){
        $data['status'] = ($status==1) ? 1: 0;
        $result = $this->pcb_news_model->updatePlugins($data,$plugin_name);
        if ($result != 0) {
            echo "1";
        }
        }
